...
export var Heatmap = function (options) {
    ...
    this.getBounds = function () {
        var parent = d3.select(this.el).select("h3"),
            width = Math.min(parent.node().offsetWidth,
                    parent.node().clientWidth) - margin.left - margin.right,
...
export var RegionMap = function (options) {
    var self = this;
    var dashboard = options.dashboard;
    ...
    this.create = function () {
        ...
        info.update = function (props) {
            if (props) {
                var displayData = dashboard.get("data.map_data")[props[self.region]],
                    text;
                ...
                self._div.innerHTML =
                    "<h4>" + capitalizeFirstLetter(
                        self.region) + " " + props[self.region] + "</h4>" +
                    "<div>" + text + "</div>";
            } else {
                self._div.innerHTML = "<div>Hover over a " + self.region + "</div>";
            }
        ...
        function toggleRegion(e) {
            var layer = e.target;
            toggleFilter(dashboard, self.region,
                dashboard.get(
                    "data." + self.region + "_ids")[layer.feature.properties[self.region]]);
        }
        ...
        d3.json(
            ...
            function (json) {
                json.features = _(json.features).reject(
                    function (d) {
                        return !d.properties[self.region];
                    });
    ...
    this._update = function (data) {
        ...
        var styleFn = function (d) {
            var n = Math.min(
                numColors - 1, Math.floor(scale(data[d.properties[self.region]])));
            return {
